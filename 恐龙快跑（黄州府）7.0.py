...
current_bgm = 0  # 当前播放的背景音乐索引

def load_and_play_bgm(index):
    """加载并播放指定索引的背景音乐"""
    try:
        pygame.mixer.music.stop()  # 停止当前播放的音乐
        pygame.mixer.music.load(bgm_tracks[index])  # 加载指定的背景音乐
        pygame.mixer.music.set_volume(0.5)  # 设置音量（0.0到1.0之间）
        pygame.mixer.music.play(-1)  # -1表示循环播放
        return True
    except pygame.error as e:
        print(f"无法加载或播放背景音乐 {bgm_tracks[index]}: {e}")
        return False

# 初始播放第一首背景音乐
load_and_play_bgm(current_bgm)
...
# 主游戏函数
def main():
    global current_bgm  # 使用全局变量跟踪当前播放的音乐
    run = True  # 游戏运行标志
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_m:  # 按M键切换背景音乐
                    current_bgm = (current_bgm + 1) % len(bgm_tracks)
                    load_and_play_bgm(current_bgm)
